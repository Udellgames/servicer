namespace Servicer
{
    ...
    public sealed class ServiceLocator
    {
        ...
        private IDictionary<Tuple<Type, object>, IList<AutoResetEvent>> waitHandles = new Dictionary<Tuple<Type, object>, IList<AutoResetEvent>>();
        ...
        [Obsolete("Promises are obsolete in .NET 4+, use GetServiceAsync instead.")]
        ...
        /// <returns>A task that returns the service when it becomes available.</returns>
        public static Task<T> GetServiceAsync<T>() where T : class
        {
            return GetServiceAsync<T>(null);
        }
        ...
        /// <returns>A task that returns the service when it becomes available.</returns>
        public static Task<T> GetServiceAsync<T>(object key) where T : class
        {
            return new TaskFactory<T>().StartNew(() =>
            {
                ...
                if (!Instance.waitHandles.ContainsKey(dictKey))
                {
                    Instance.waitHandles.Add(dictKey, new List<AutoResetEvent>());
                }
                ...
                Instance.waitHandles[dictKey].Add(waitHandle);
                ...
                {
                    var subTypeKey = Instance.services.Keys.First(
                        x =>
                            (key == null ? x.Item2 == null : key.Equals(x.Item2)) &&
                            type.IsAssignableFrom(x.Item1));
        ...
        public static void Register<T>(T service, object key) where T : class
        {
            ...
            else
            {
                ...
                IList<AutoResetEvent> waitHandles;
                if (Instance.waitHandles.TryGetValue(dictKey, out waitHandles))
                {
                    ...
                    Instance.waitHandles.Remove(dictKey);
                }
                ...
                if (!Explicit)
                {
                    ...
                    subTypeKeys = Instance.waitHandles.Keys.Where(
                        x =>
                            (key == null ? x.Item2 == null : key.Equals(x.Item2)) &&
                            x.Item1.IsAssignableFrom(type));
                    ...
                    foreach (var subTypeKey in subTypeKeys.ToList())
                    {
                        if (Instance.waitHandles.TryGetValue(dictKey, out waitHandles))
                        {
                            ...
                            Instance.waitHandles.Remove(subTypeKey);
                        }
                    }
                }
            }
